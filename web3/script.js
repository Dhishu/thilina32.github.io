function schange(a2){
    ...
    if(window.scrollY > (wh*2) / 1.2 && window.scrollY < (wh*4) / 1.2){
        if(a == 1 || a == 3 || a2==1){
            ...
            d2.innerHTML ='<div class="co"><div><span>Full name</span><input type="text"></div><div><span>Email</span><input type="text"></div><div><span>Contact numder(whatsapp)</span><input type="text"></div><div><span>Title</span><input type="text"></div><div><span>Dr</span><textarea type="text"></textarea></div><span><div class="bu bu2">send</div></span></div>';
            d3.height='100vh';
